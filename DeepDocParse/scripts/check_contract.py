#!/usr/bin/env python
"""契约守卫：gateway 实际暴露的端点必须与 openapi.yaml 一字不差。

铁律 4（改 `/v1/*` 行为必须先改 openapi.yaml）此前只写在文档里，没有任何机械保障。
这个脚本把它变成可执行的检查，CI 每次都跑：

- gateway 有、契约没有  -> 端点在契约外偷偷长出来了。DeepDocParse-Web 只认契约，
  它看不见的端点等于不存在；而契约已冻结 v1.0，新增必须先落到 openapi.yaml。
- 契约有、gateway 没有  -> 承诺了没实现。调用方按契约写代码会拿到 404。

只管路径与方法这一层。响应体的形状由 tests/test_contract.py 管，两者互补。
...
def gateway_endpoints() -> set[tuple[str, str]]:
    ...
    """
    from app.main import app          # noqa: PLC0415 —— 必须在 sys.path 就位之后
...
def main() -> int:
    ...
    declared = contract_endpoints(spec_path)
    ...
    for path, method in undeclared:
        print(f"::error::gateway 暴露了契约里没有的端点：{method.upper()} {path}\n"
              f"  铁律 4：改 /v1/* 行为必须先改 openapi.yaml（契约已冻结 v1.0，"
              f"只许向后兼容的新增）")
    for path, method in unimplemented:
        print(f"::error::openapi.yaml 声明了 gateway 没实现的端点："
              f"{method.upper()} {path}\n"
              f"  调用方按契约写代码会拿到 404")

    if undeclared or unimplemented:
        return 1
    print(f"契约守卫通过：{len(declared)} 个端点，openapi.yaml 与 gateway 一致")
    return 0
