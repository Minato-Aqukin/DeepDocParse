...
cd DeepDocParse-Web
./quickstart.sh --host <服务器公网IP或域名> --chat-url http://127.0.0.1:11434/v1 --chat-model qwen3:8b -y
```

跑完打开 `http://<你的IP>`，注册第一个账号即可。

`quickstart.sh` 依次做四件事，每一件都能单独重跑：
...
```bash
./quickstart.sh configure --chat-url http://127.0.0.1:11434/v1 --chat-model qwen3:8b
...
```bash
./quickstart.sh systemd     # 需要 root/sudo
```
...
```bash
./quickstart.sh doctor          # 密钥 / 引擎名一致性 / 容器回访 / 探针
./quickstart.sh status          # 容器、进程、各层探针
./quickstart.sh logs backend    # 也可以 gateway | arq-worker | embed | postgres | edge
```
...
```bash
git pull && ./quickstart.sh build && ./quickstart.sh restart
```
...
| `.quickstart/ddp-web.service` | systemd 单元（`./quickstart.sh systemd` 才会装到系统里） | 否 |
