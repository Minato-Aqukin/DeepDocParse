...
| **多记录行准确率** | 顶层 `array` 逐行精确匹配；忽略顺序，重复行按次数计，缺行/多行都扣分 | 期望含 `records` 的样本 |
...
顶层 `array` 的记录顺序不是语义：同三行货品换个顺序仍是同一份结果。
判定因此按**多重集合**匹配：每条期望行最多消费一条预测行；重复行必须重复出现；
漏一行与多编一行都会各记一次失败。字段集合或任一字段值不同，整行判错。
它不混进普通字段准确率，避免“一个 200 列表格”把对象字段的指标完全淹没。

## 当前结论（2026-08-27，offline，6 条种子样本）

| 切片 | 样本 | 字段准确率 | 出处命中率 | 空值正确率 | 多记录行准确率 | schema 合规率 |
|---|---|---|---|---|---|---|
| 全部 | 6 | — | 88.9% (8/9) | — | —（offline 不调模型） | 100.0% (6/6) |
...
- **多记录（array）的 live 数字**：逐行判定已经实现，但本机没有可用的 chat + embedding
  运行时，`contract/schedule-rows` 的真实行准确率仍要随 GPU 批次一补上。
