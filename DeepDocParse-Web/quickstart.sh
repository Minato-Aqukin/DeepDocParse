...
#   cd DeepDocParse-Web && ./quickstart.sh
#
...
#
# 用法：./quickstart.sh [子命令] [选项]，./quickstart.sh help 看全部选项。
set -euo pipefail
...
save_state() {
  mkdir -p "$STATE_DIR"
  {
    echo "# 由 quickstart.sh 生成，记住上次的部署参数；改这里不如重跑 configure"
...
write_registry() {
  ...
  {
    echo "# 由 quickstart.sh 生成 —— 不要手改，重跑 ./quickstart.sh configure 会覆盖。"
...
write_service_override() {
  ...
  {
    echo "# 由 quickstart.sh 生成 —— 叠在 DeepDocParse/docker/compose.cpu.yml 之上。"
...
# 由 quickstart.sh 生成 —— 重跑 ./quickstart.sh configure 会覆盖。
...
# 单元文件先生成好放着，装不装由 `./quickstart.sh systemd` 决定（那一步要 root）
write_systemd_unit() {
...
# 由 quickstart.sh 生成。安装：./quickstart.sh systemd
[Unit]
...
UNIT
  ok "$SYSTEMD_UNIT_FILE（要开机自启就跑 ./quickstart.sh systemd）"
}
...
cmd_systemd() {
  ...
  [ -f "$SYSTEMD_UNIT_FILE" ] || die "缺 $SYSTEMD_UNIT_FILE，先跑 ./quickstart.sh configure"
...
cmd_models() {
  ...
  find_python || die "找不到 >=3.11 的 python（先跑 ./quickstart.sh deps）"
...
cmd_start() {
  ...
  load_state
  check_stack_conflict
  [ -f "$WEB_ENV" ] || die "还没配置过，先跑 ./quickstart.sh configure"
  [ -f "$SERVICE_OVERRIDE" ] || die "缺 $SERVICE_OVERRIDE，先跑 ./quickstart.sh configure"
  [ -x "$WEB_DIR/.venv/bin/python" ] || die "backend venv 不存在，先跑 ./quickstart.sh build"
  [ -f "$WEB_DIR/frontend/dist/index.html" ] || die "前端还没构建，先跑 ./quickstart.sh build"
  ...
  dim "自检：./quickstart.sh doctor   看日志：./quickstart.sh logs backend"
}
...
cmd_doctor() {
  ...
  if [ "$SKIP_MODELS" = 1 ]; then
    ...
    fail "缺 $MODELS_DIR/bge-m3/model.safetensors（跑 ./quickstart.sh models）"; DOCTOR_FAILED=1
  fi
...
示例
  ./quickstart.sh --host 203.0.113.10 --chat-url http://127.0.0.1:11434/v1 --chat-model qwen3:8b -y
  ./quickstart.sh configure --port 8000 && ./quickstart.sh restart
HELP
}
...
parse_args() {
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      *)                 die "不认识的选项：$1（./quickstart.sh help 看用法）" ;;
    esac
  done
}
...
main() {
  ...
  case "$cmd" in
    ...
    systemd)   cmd_systemd ;;
    help)      cmd_help ;;
    *)         die "不认识的子命令：$cmd（./quickstart.sh help 看用法）" ;;
  esac
}
