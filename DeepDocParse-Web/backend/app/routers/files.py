...
from app.models import FileToken, Task, as_aware
from app.storage import Storage

router = APIRouter()

# 只有这些类型允许 inline 预览（前端 iframe 要用）。HTML/SVG/XML 一律不在内 ——
# 它们能在本站同源执行脚本。
INLINE_SAFE_MIMES = {
...
async def get_file(token: str, session: AsyncSession = Depends(get_session),
    ...
    if task is None or not task.object_key:
        raise APIError(404, "file not found", "invalid_request_error", "file_not_found")
    try:
        data = await storage.get(task.object_key)
    except Exception:
        raise APIError(404, "file object missing", "invalid_request_error", "file_not_found")

    # 安全要点：task.mime 来自上传方的 multipart 头，完全可控。原样 inline 回去
    # 等于允许上传 text/html 并在本站同源执行脚本（token 一转发就能偷走别人
    # localStorage 里的 JWT）。因此：白名单之外一律当二进制附件下载，并禁止嗅探。
    inline = task.mime in INLINE_SAFE_MIMES
    headers = {
    ...
    if not inline:
        ...
        # 会把前端 iframe 预览一起弄坏（本机内置浏览器不渲染 PDF，这条未能实测，故取保守解）
        headers["Content-Security-Policy"] = "sandbox; default-src 'none'"
    return Response(content=data,
                    media_type=task.mime if inline else "application/octet-stream",
                    headers=headers)
