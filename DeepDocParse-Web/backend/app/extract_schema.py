"""DDP-Extract v1 —— schema 解析、值强制转换、结果自检（产品层实现）。

**这是 service 侧 `gateway/app/services/extract_format.py` 的同源实现**，
按铁律 1 各写一份：本层不 import service 的代码，耦合面只有解析契约与
OpenAI 兼容端点两处。格式契约本身写在 ../DeepDocParse/docs/extract-format.md，
**改字段先改那份文档，两边一起改**。

这一层只做纯函数，不碰数据库也不碰上游 —— schema 的边界因此能被单测直接钉住。

与版面 normalizer 的关键差别：`validate_schema` **在请求路径上强制**。
版面自检不强制是因为"一个字段缺失不该让整份解析结果作废"；
但 schema 是用户给的输入，坏输入要当场 400 拒掉 ——
跑完一轮抽取（N 次检索 + N 次模型调用）再说不合规，是在烧用户的额度。
"""
import json
import re
