...
- 往 YAML 里插重复键去改配置 —— `yaml.safe_load` 静默取最后一个，
  配置根本没变，守卫当然绿；
- 想把一条外键加回去复现缺陷 —— `ADD CONSTRAINT` 会校验历史行，
  而库里已经有违反它的数据，**ALTER 直接失败**，约束压根没加上。
  （正确做法是 `NOT VALID`：对新插入生效，不校验历史行。）

两次都差点据此把一条好守卫判成假的。**判定"假守卫"之前，先确认变异真的
生效了** —— 查一下改完的值/`pg_constraint`/文件内容，别只看守卫的颜色。
