...
## F-31 · 跨服务的守卫住进了单个包的测试里

**严重度**：中（守卫在本机绿、在 CI 红，而它守的东西是对的）

「向量索引名的写入侧、检索侧、`ddp_core` 三边必须是同一个函数对象」——
这条判据本身很好（比"算出的字符串相等"强：后者在两份实现漂开、
而当前维度恰好算出同一个名字时会绿）。它防的是历史上那个
「换 embedding 模型后维度不符的向量被 RediSearch 静默丢弃 →
永久零命中、永久退回 BM25」。

但它住在 `services/mcp/tests/`，而它 import `ddp_gateway` ——
mcp 那个 CI 作业**不装 model-gateway**，也不该装（mcp 不依赖网关）。
于是：本机绿（venv 里什么都有）、CI 红 `No module named 'ddp_gateway'`。

而 `tests/test_architecture_guards.py` 开头那段说明写的正是这件事：
「横跨多个包的守卫放进任何一个包里，都会变成**只在那个包的 CI 里跑**」。

**处理**：搬去仓库根的 `tests/`，而不是给 mcp 的安装线加一个它本来不需要
的依赖 —— 后者会让「mcp 不依赖网关」这条架构约束在 CI 里失去证明。
