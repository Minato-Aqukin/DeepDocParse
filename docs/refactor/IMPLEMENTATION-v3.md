...
## 2026-09-16 协调者 operation 闭集 + Wiki 列表组织边界（跨来源 Wiki 调研抓到的既有缺陷）

两条都是**静默错义**，不是报错：

- **G1 operation 不闭集**：`create_plan` 只看"本地生成是否就绪"，不看 `TaskSpec.operation`，
  于是任何 operation 都会被追加一个 `answer` 步 —— 提交 `corpus.retrieve`（只取证据）会白跑
  一次生成并把结果当成答案，提交一个没实现的 operation（如 `wiki.pages`）会拿回一个 RAG 答案。
  现在：契约新增闭集 `federation_task_operation`（`corpus.retrieve` / `rag.answer.cited`），
  `create_intent` 认不出来当场 400 `capability_unsupported`、不落库；不要答案时既不问本地生成能力
  （省掉一次能力探测），也不留生成预算；`_answer_result` 对只取证据的任务返回**没有原因的空答案**
  （"没模型"和"你没要答案"是两件事，界面据此说"这个任务只取证据"）。
  `TaskListItem.operation` 绑定该枚举，前端类型改用生成物。
- **G6 Wiki 列表缺组织边界**（不变式 8）：已发布分支没有组织谓词，靠后面逐条 `revision_out`
  的 404 兜底 —— 别的组织的行会占掉 `limit 200` 的名额，本组织的 Wiki 因此可能根本不出现在
  列表里。用例按"别处有 200 条更新的已发布 Wiki"钉住，去掉谓词即红（第一版只断言"别的组织
  看不到"，旧代码靠 404 兜底照样绿，是假测试，已改）。
- **生成器放宽**：`enums.yaml` 的取值允许点分段（`rag.answer.cited`），常量名里点换成下划线
  （`ident()`），TS 的 META 键改成带引号 —— 否则这类 operation 名进不了枚举，只能在三处各写一份。
  同时加一道校验：同一枚举里 `a.b` 与 `a_b` 会生成同一个常量名（Go 侧是两条重复声明，
  `--check` 与 gofmt 都看不出来，只有 `go build` 才报），现在当场报错。
- **契约 YAML 的重复键当场报红**（`scripts/contract_yaml.py`，四个读取口共用：契约守卫、
  联邦路由守卫、联邦契约守卫、枚举生成器）：
  本次验收就抓到一处 —— 同一个操作下写了两段 `description:`，`yaml.safe_load` 静默只留最后一个，
  新写的闭集说明**整段没进契约**，而且没有任何工具会红。这个项目此前已被"重复键静默取最后一个"
  坑过一次（当时是变异没生效）。
- **`get_wiki` 也补组织谓词**：已发布 Wiki 的跨组织直读此前靠依赖资源那一层间接挡住，
  防线更薄 —— 用一份**没有依赖行**的已发布 Wiki 钉住（将来真允许无依赖 Wiki 时就是这条路径漏），
  去掉谓词即红。


## 2026-09-16 Web 联邦任务：创建、批准与取消/续跑（改进项 ① 的 a2-2）

两层许可第一次在网页上真正由用户签出来。界面只组装，**不代签、不代补**。

- **创建任务**（`components/federation/TaskComposer.vue`，挂在 `/tasks` 的"新建任务"里）：
  问题、要什么（带出处的回答 / 只取证据）、查哪些资料（本站公开 / 指定资源 / 联邦公开范围）、
  检索方式、外发许可。联邦范围先 `POST /api/v1/federation/scopes` 封存一份清单 ——
  **穷查只在拿到已封存清单时才可选**，否则"查全了"没有分母。
  建完直接进详情页（规划状态与批准入口都在那边）。
- **外发许可（第一层）**：只有范围清单里存在远端来源节点时才出现。
  可勾的载荷逐项列出，**问题原文也是其中一项，默认勾上但可以取消** ——
  取消了就真的一个字节都不发，对应远端目标由协调者如实记成 denied，
  界面不替用户补回去。没有远端接收方时一律 `local_only` + 载荷/接收方/预算全空（契约 allOf）。
- **批准计划（第二层）**（`components/federation/PlanApproval.vue`）：
  批准前把这一份修订实际会发出去的东西摆全 —— 接收方（**含中继**）、外发内容、保留策略、有效期。
  许可对象完全由计划推出（`recipientsOf` / `retentionOf`），与协调者 `approve` 的判据逐项一致：
  少一个端点就 `egress_denied`，多一个就是批准了计划里没有的外发。
  各边保留策略不一致时**报出来并禁用按钮**，不替用户挑一个。
  已批准的计划只受理不重签（重签会把 `granted_at` 悄悄改成现在）。
- **取消与续跑**：判据与协调者一致。取消用契约的 `active` 标记（与轮询同一个判据，
  契约新增进行中状态时不会把它当终态）；续跑要求 `planning_state === 'approved'` ——
  **少了这一条，规划阶段就失败的任务会显示一个点下去必定 403 的按钮**。
  续跑会重新规划修订，所以点完丢掉本地计划重读，不拿旧摘要去批。
- **写在前端的两个判断，是照协调者的代码对出来的，不是想当然**：
  ① **执行策略看范围种类，不看有没有远端目标** —— `plans.validate_spec` 里 `local_only` 执行
  禁止 `federation_public` 范围，而联邦清单完全可能只枚举到本节点；按"有没有远端"选执行策略的话，
  这种任务在协调者那当场被拒。② 外发许可反过来只看有没有远端接收方。两条各有回归用例，
  变异（把 ① 改回"看远端"）当场红。
- **"还没受理过"的判据**：受理会立刻把状态推到 `running`，所以 `queued` 唯一地表示
  "计划还没提交执行"——权威标记（受理幂等键）是内部字段，状态响应里没有。批准入口只在这个窗口给。
- **两张载荷文案表对着 schema 文件逐字比对**（`src/constants/__tests__/federation.spec.ts`）：
  `allowed_payload` 与 `payload_kind` 内联在 JSON schema 里、不是 `enums.yaml` 的生成物，
  前端只能抄一份。漏抄的后果是静默的 —— 用户在界面上勾不到那一类，对应目标被记成 `denied`，
  看起来像"对方拒绝了"。删掉一条即红。
- **夹具**：新增 `scope-sealed.json` / `plan-ready.json` / `task-queued.json`，
  全部登记进 `test_web_federation_fixtures.py`。`ScopeEnvelope` 跨文件引用
  `ddp-scope-coverage` 的 `$defs`，把两份 schema 接进同一个文档再验 —— 不接的话
  jsonschema 解不开那个 `$ref`，整个 manifest 会被当成无约束对象放过去（删一个必填字段即红）。
  顺带让 `scope-v1.yaml` 第一次有了重复键防线（此前没有任何守卫读它）。
  计划的有效期在 e2e 里按真实时钟改写，否则用例会在夹具里那个时刻之后永远红。
- **没做**：交付字节的下载与本地校验确认（`/deliveries/{id}` 与 `ack`）；
  跨修订的重新规划入口（现在只能靠续跑触发）。
