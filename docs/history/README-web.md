...
```
DeepDocParse-Web/
├── init.sh      # 唯一初始化入口（local / docker / autodl）
├── start.sh     # 唯一运行入口（start / stop / status / logs / doctor）
...
cd DeepDocParse-Web
./init.sh docker --host <服务器IP或域名> --chat-url http://127.0.0.1:11434/v1 --chat-model qwen3:8b -y
```

跑完再执行 `./start.sh docker start`，打开 `http://<你的IP>` 注册第一个账号即可。
`init.sh` 从零装依赖、
clone service 仓库、生成两份 `.env`（随机密钥）、按 CPU/内存/GPU 调参、下模型权重、
构建前端、起全套服务，最后自检一遍。四步也能单独跑：

```bash
./init.sh docker configure   # 环境配置：两份 .env + 前端 + 注册表 + nginx
./init.sh docker tune        # 优化配置：按硬件定并发/批量/上传上限
./init.sh docker models      # 模型权重：bge-m3（TEI 只认 safetensors，会自动转）
./start.sh docker start       # 服务启动：容器 -> 迁移 -> backend -> nginx
./start.sh docker doctor      # 自检：密钥、引擎名一致性、容器回访、探针
```
